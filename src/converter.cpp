        ...
        try
        {
            for (const auto &entry: getEntriesInAlphabeticalOrder(stringsGetter.getInputDirPath()))
            {
                if (std::filesystem::is_regular_file(entry))
                {
                    ...
                    if (static_cast<std::string>(entry.path()).find(".xml") == std::string::npos and
                        static_cast<std::string>(entry.path()).find(".tot") == std::string::npos and
                        static_cast<std::string>(entry.path()).find(".totj") == std::string::npos)
                        continue;
                    ...
                    if (file.is_open())
                    {
                        try
                        {
                            ...
                            if (filename.size() >= 4 and filename.compare(filename.size() - 4, 4, ".tot") == 0)
                                j = getJsonFromTOFile(file);
                            else if (filename.size() >= 5 and filename.compare(filename.size() - 5, 5, ".totj") == 0)
                                j = getJsonFromTOJFile(file);
                            else if (filename.size() >= 4 and filename.compare(filename.size() - 4, 4, ".xml") == 0)
                                j = getJsonFromFileStream(file);
                            else
                            {
                                std::cerr << BHRED << "Error: " << entry.path() << " is not a valid file" << rstColor
                                          << std::endl;
                                return EXIT_FAILURE;
                            }

                            std::string nameTA = getStringGivenPosAndToken(getWordAfterLastSymbol(entry.path(), '/'), '.', 0);
                            std::string outputFileName = nameTA + ".tck";
