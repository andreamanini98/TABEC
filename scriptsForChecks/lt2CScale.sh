...
# ------------------------
# Start the 'sed' command with extended
# regular expressions enabled.
# Extended regular expressions allow for
# features like grouping and quantifiers.

#   sed -E

# Condition: If the line does not start with
# '#', apply the following substitution command:
...
# A capturing group that matches either a blank
# space or the start of the line (^).

#   ( |^)

# A capturing group that matches an integer, starting
# with a non-zero digit (1-9) followed by zero or more
# other digits (0-9).

#   ([1-9][0-9]*)

# A capturing group that matches either a blank space,
# a closing curly brace '}', or the end of the line ($).
...
# The 'g' flag indicates that the substitution should be
# performed globally on each line.
